defmodule UniversalProxy.SSHAccess do
  @moduledoc """
  Device-managed SSH access key.

  On first boot the proxy generates a single ed25519 keypair, persists it,
  and authorizes its **public** half for SSH logins (via
  `NervesSSH.add_authorized_key/1`). The operator downloads the matching
  **private** half from the Security tab and uses it as their SSH identity:

      ssh -i universal_proxy root@<hostname>.local
  ...
  alias UniversalProxy.SSHAccess.Keypair

  @ed25519_oid {1, 3, 101, 112}
  @comment ~c"root@universal-proxy"
  @key :keypair
